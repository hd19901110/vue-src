  ...
  <jee-aside-layout class="smart-container">
    <section slot="asideHeader">
      <el-button
        ...
        class="new-grouping"
        :disabled="!this._checkPermission('/cmsadmin/formType', 'POST')"
...
export default {
        ...
        buttons: [
          {
            ...
            text: '新建表单',
            icon: 'xinjian',
            disabled: !this._checkPermission('/cmsadmin/smartForm', 'POST')
          },
          {
            type: 'Issue',
            text: '发布',
            icon: 'fabu',
            disabled: !this._checkPermission('/cmsadmin/smartForm/publish', 'PUT')
          },
          {
            type: 'Stop',
            text: '暂停',
          ...
          },
          {
            ...
            text: '删除',
            icon: 'delete',
            disabled: !this._checkPermission('/cmsadmin/smartForm', 'DELETE')
          }
        ]
      },
      searchHeader: {
        ...
        handleColumn: [
          {
            ...
            name: '编辑字段',
            icon: 'bianji',
            disabled: !this._checkPermission('/cmsadmin/smartForm/updateFields', 'POST')
          },
          {
            type: 'Issue',
            name: '发布',
          ...
          },
          {
            ...
            buttons: [
              ...
              },
              {
                ...
                name: '复制',
                icon: 'fuzhi',
                disabled: !this._checkPermission('/cmsadmin/smartForm/copy', 'POST')
              },
              {
                type: 'Delete',
                name: '删除',
              ...
              },
              {
                ...
                disabled: !this._checkPermission('/cmsadmin/smartFormData/export', 'GET')
              },
              {
                type: 'Empty',
                name: '清空结果',
                icon: 'delete',
                disabled: !this._checkPermission('/cmsadmin/smartForm/clearData', 'DELETE')
              }
            ]
          }
        ],
  ...
  methods: {
    ...
    handleSmartSelect (data) {
      ...
      if (ids.length > 0) {
        if (viewStatus.find(v => v === 1 || v === 2)) {
          ...
          this.smartHeaders.buttons.find(v => v.type === 'Issue').disabled = !this._checkPermission('/cmsadmin/smartForm/publish', 'PUT')
        }
      } else {
        this.smartHeaders.buttons.find(v => v.type === 'Issue').disabled = !this._checkPermission('/cmsadmin/smartForm/publish', 'PUT')
      }
      ...
      if (ids.length > 0) {
        if (viewStatus.find(v => v === 0) >= 0) {
          ...
          this.smartHeaders.buttons.find(v => v.type === 'Stop').disabled = !this._checkPermission('/cmsadmin/smartForm/publish', 'PUT')
        }
      } else {
        this.smartHeaders.buttons.find(v => v.type === 'Stop').disabled = !this._checkPermission('/cmsadmin/smartForm/publish', 'PUT')
      }
    ...
    handleTableDerive (row) {
      ...
      window.open(`${this.$path}/cmsadmin/smartFormData/export?formId=${formId}&JEECMS-Auth-Token=${token}`)
    },
    // 表格清空结果
