        ...
        handleColumn: [
          {
            ...
            name: '发布',
            icon: 'fabu',
            disabled: !this._checkPermission('/cmsadmin/questionnaire/release', 'PUT'),
            iconStyle: {
              fontSize: '14px'
            }
          },
          {
            ...
            name: '暂停',
            icon: 'ai06',
            disabled: !this._checkPermission('/cmsadmin/questionnaire/pause', 'PUT'),
            iconStyle: {
              fontSize: '14px'
            }
          },
          {
            ...
            name: '复制',
            icon: 'fuzhi',
            disabled: !this._checkPermission('/cmsadmin/questionnaire/copy', 'POST'),
            iconStyle: {
              fontSize: '14px'
            }
          },
          {
            ...
            name: '预览',
            icon: 'yulang',
            disabled: !this._checkPermission('/cmsadmin/questionnaire', 'GET')
          },
          {
            type: 'Edit',
            name: '编辑',
          ...
          },
          {
            ...
            name: '删除',
            icon: 'delete',
            disabled: !this._checkPermission('/cmsadmin/questionnaire', 'DELETE')
          }
        ],
        handleColumnProp: {
          label: '操作',
        ...
        buttons: [
          {
            ...
            disabled: !this._checkPermission('/cmsadmin/questionnaire', 'POST')
          },
          {
            type: 'SubmitReview',
            text: '提交审核',
            icon: 'shenhe1',
            disabled: !this._checkPermission('/cmsadmin/questionnaire/submit', 'PUT'),
            isHidden: ''
          },
          {
            type: 'Issue',
    ...
    handleMultipleSelect (data) {
      ...
      if (ids.length > 0) {
        if (status.find(v => v !== 0 && v !== 2)) {
          ...
          this.headers.buttons.find(v => v.type === 'SubmitReview').disabled = !this._checkPermission('/cmsadmin/questionnaire/submit', 'PUT')
        }
      } else {
        this.headers.buttons.find(v => v.type === 'SubmitReview').disabled = !this._checkPermission('/cmsadmin/questionnaire/submit', 'PUT')
      }
      ...
      if (this.workFlow) {
        if (ids.length > 0) {
          if (data.find(v => v.status !== 0 || v.checkStatus === false)) {
            ...
            this.headers.buttons.find(v => v.type === 'Issue').disabled = !this._checkPermission('/cmsadmin/questionnaire/release', 'PUT')
          }
        } else {
          this.headers.buttons.find(v => v.type === 'Issue').disabled = !this._checkPermission('/cmsadmin/questionnaire/release', 'PUT')
        }
      ...
      if (ids.length > 0) {
        if (status.find(v => v !== 3 && v !== 4) >= 0) {
          ...
          this.headers.buttons.find(v => v.type === 'Pause').disabled = !this._checkPermission('/cmsadmin/questionnaire/pause', 'PUT')
        }
      } else {
        this.headers.buttons.find(v => v.type === 'Pause').disabled = !this._checkPermission('/cmsadmin/questionnaire/pause', 'PUT')
      }
