<template>
  <section class="vip-grade-container">
    ...
    <template #levelName="scope">
      <el-link :underline="false" :disable="!_checkPermission('/cmsadmin/memberLevel', 'GET')" class="jee-color" @click="rowHref(scope.scope.row)">{{scope.scope.row.levelName}}</el-link>
    </template>
    </base-table>
    <form-dialog
      :title="dialogVip"
        ...
        handleColumn: [
          {
            ...
            name: '删除',
            icon: 'delete',
            disabled: !this._checkPermission('/cmsadmin/memberLevel', 'DELETE')
          }
        ],
        handleColumnProp: {
          width: '75px'
        ...
        buttons: [
          {
            ...
            text: '新建',
            icon: 'xinjian',
            disabled: !this._checkPermission('/cmsadmin/memberLevel', 'POST')
          },
          {
            type: 'Delete',
            text: '删除',
      ...
      return [
        {
          text: '保存',
          type: 'Submit',
          disabled: !this._checkPermission('/cmsadmin/memberLevel', this.addForm.id ? 'POST' : 'PUT')
        }
      ]
    }
  },
