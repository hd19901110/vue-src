...
export default {
    ...
    return {
      ...
      form: {
        ...
        submitBtns: [{
          text: '保存',
          subType: 'submit',
          disabled: !this._checkPermission('/cmsadmin/messageTpl', this.linkType === 'add' ? 'POST' : 'PUT')
        }]
      },
      emailForm: {
        form: {
      ...
      phoneForm: {
        ...
        submitBtns: [{
          text: '保存',
          subType: 'submit'
          // disabled: !this._checkPermission('/cmsadmin/messageTpl', 'POST')
        }]
      },
      mailForm: {
        form: {
  ...
  methods: {
    ...
    getFormData () {
      ...
      this.linkType = type
      this.emailForm.submitBtns[0].disabled = !this._checkPermission('/cmsadmin/messageTpl', this.linkType === 'add' ? 'POST' : 'PUT')
      this.phoneForm.submitBtns[0].disabled = !this._checkPermission('/cmsadmin/messageTpl', this.linkType === 'add' ? 'POST' : 'PUT')
      this.mailForm.submitBtns[0].disabled = !this._checkPermission('/cmsadmin/messageTpl', this.linkType === 'add' ? 'POST' : 'PUT')
