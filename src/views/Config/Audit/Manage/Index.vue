  ...
  <section class="audit-manage-index">
    ...
    <template #name="scope">
      ...
      <div class="nameBox" v-else>
        ...
        <div @click="editName(scope.scope.row)" :class="[{'event':!_checkPermission('/cmsadmin/auditStrategy/name', 'PUT')},'hide']">
          <jee-icon iconClass="bianji" :class="['icon', {'jee-popover':_checkPermission('/cmsadmin/auditStrategy/name', 'PUT')}]"></jee-icon>
        </div>
      </div>
    </template>
    <template #textAuditScene='scope'>
      <div :class="['jee-popover','fcs','jee-hover-color',{'jee-click':scope.scope.row.isText}]">
          <el-popover
            ...
            :visible-arrow='false'
            :class="{'event':!_checkPermission('/cmsadmin/auditStrategy/scene/text', 'PUT')}"
            trigger="click">
            <el-checkbox-group
              v-model="textChecked"
    ...
    <template #pictureAuditScene='scope'>
      <div :class="['jee-popover','fcs',{'jee-click':scope.scope.row.isImage}]">
        <el-popover
          ...
          :visible-arrow= 'false'
          :class="{'event':!_checkPermission('/cmsadmin/auditStrategy/scene/picture', 'PUT')}"
          trigger="click">
          <el-checkbox-group
  ...
  data () {
    ...
    const columns = [
      ...
      },
      {
        ...
        enableText: '开启',
        disableText: '关闭',
        disabled: !this._checkPermission('/cmsadmin/auditStrategy/open', 'GET')
      }
    ]
    return {
      temporaryText: [],
        ...
        handleColumn: [
          {
            ...
            name: '删除',
            icon: 'delete',
            disabled: !this._checkPermission('/cmsadmin/auditStrategy', 'DELETE')
          }
        ],
        handleColumnProp: {
          label: '操作',
        ...
        buttons: [
          {
            ...
            text: '创建策略',
            icon: 'xinjian',
            disabled: !this._checkPermission('/cmsadmin/auditStrategy', 'POST')
          },
          {
            type: 'Delete',
            text: '批量删除',
