...
export default {
        ...
        handleColumn: [
          {
            ...
            icon: 'zhifeiji-aqg',
            disabled: !this._checkPermission('/cmsadmin/wechatfans/send', 'POST')
          }
        ],
        handleColumnProp: {
          label: 'global.handle',
      ...
      submitBtns: [
        {
          text: '设置标签',
          bthType: 'SetTags',
          disabled: !this._checkPermission('/cmsadmin/wechattags/set', 'POST')
        },
        {
          text: '同步所有粉丝',
          bthType: 'SyncFanAll',
          disabled: !this._checkPermission('/cmsadmin/wechatfans/sync', 'GET')
        },
        {
          text: '同步选中粉丝',
          bthType: 'SyncFans',
          disabled: !this._checkPermission('/cmsadmin/wechatfans/syncFans', 'POST')
        },
        {
          text: '加入黑名单',
          bthType: 'Blacklist',
          disabled: !this._checkPermission('/cmsadmin/wechatfans/black', 'POST')
        }
      ],
      edit: {
        title: '设置标签',
  ...
  methods: {
    ...
    handleSelectionChange (data) {
      ...
      if (ids.length > 0) {
        this.submitBtns.find(v => v.bthType === 'SyncFans').disabled = !this._checkPermission('/cmsadmin/wechatfans/syncFans', 'POST')
        this.submitBtns.find(v => v.bthType === 'Blacklist').disabled = !this._checkPermission('/cmsadmin/wechatfans/black', 'POST')
      } else {
        this.submitBtns.find(v => v.bthType === 'SyncFans').disabled = true
        this.submitBtns.find(v => v.bthType === 'Blacklist').disabled = true
      }
