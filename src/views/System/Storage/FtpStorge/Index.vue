  ...
  <section class="security-index-container">
    ...
    >
      <template #ftpName="scope">
        <el-link :underline="false" :disable="!_checkPermission('/cmsadmin/ftp', 'GET')" class="jee-color" @click="rowHref(scope.scope.row)">{{scope.scope.row.ftpName}}</el-link>
      </template>
    </base-table>
    <form-dialog
      ref="addDialog"
...
export default {
        ...
        handleColumn: [
          {
            ...
            name: '删除',
            icon: 'delete',
            disabled: !this._checkPermission('/cmsadmin/ftp', 'DELETE')
          }
        ],
        handleColumnProp: {
          width: '75px'
        ...
        buttons: [
          {
            ...
            text: '新建',
            icon: 'xinjian',
            disabled: !this._checkPermission('/cmsadmin/ftp', 'POST')
          },
          {
            type: 'Delete',
            text: '删除',
  ...
  methods: {
    ...
    async handleConfirmEdit (data) {
      if (data.password) {
        data.password = await loginType(data.password)
      }
      this.editFormLoading = true
      await this.$request.fetchFtpStorageUpdate(data).then(res => {
        if (res.code === 200) {
